...
cleanup() {
  rm -f "${CHANGED_FILES_LIST_FILE}" "${DAILY_META_FILE:-}" "${STATS_META_FILE:-}" "${USER_DEX_GET_META_FILE:-}" "${USER_DEX_PATCH_META_FILE:-}" "${USER_DEX_SHARED_GET_META_FILE:-}" "${SETTINGS_GET_META_FILE:-}" "${SETTINGS_PATCH_META_FILE:-}" "${COGNITO_METRICS_META_FILE:-}" "${DAILY_BUNDLE_FILE:-}" "${STATS_BUNDLE_FILE:-}" "${USER_DEX_GET_BUNDLE_FILE:-}" "${USER_DEX_PATCH_BUNDLE_FILE:-}" "${USER_DEX_SHARED_GET_BUNDLE_FILE:-}" "${SETTINGS_GET_BUNDLE_FILE:-}" "${SETTINGS_PATCH_BUNDLE_FILE:-}" "${COGNITO_METRICS_BUNDLE_FILE:-}"
}
trap cleanup EXIT
